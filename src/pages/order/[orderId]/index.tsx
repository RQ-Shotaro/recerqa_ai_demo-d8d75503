...
const OrderDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (purchaseOrderError && purchaseOrderError.code !== "PGRST116") {
        ...
        if (purchaseOrderData) {
          ...
          const { data: purchaseItemsData, error: purchaseItemsError } =
            await supabase
              ...
              .select("*")
              .eq("purchase_order_id", purchaseOrderData.purchase_order_id);
  ...
  if (error) {
    return (
      <div className="min-h-screen h-full flex justify-center items-center text-red-500">
        エラーが発生しました: {error}
      </div>
    );
  }
    ...
    <div className="min-h-screen h-full bg-gray-100 flex">
      <aside className="w-64 bg-gray-800 text-white p-4 flex flex-col">
        <div className="mb-8">
          <Link
            href="/"
            className="flex items-center mb-4 px-4 py-2 hover:bg-gray-700 rounded"
          >
        ...
        <div className="bg-white shadow rounded p-4">
          {order && (
            <div className="mb-6">
              ...
              <p>
                <strong>注文ID:</strong> {order.order_id}
              </p>
              <p>
                <strong>注文日:</strong> {order.order_date}
              </p>
              <p>
                <strong>顧客ID:</strong> {order.customer_id}
              </p>
              <p>
                <strong>注文ステータス:</strong> {order.order_status}
              </p>
            </div>
          )}
          ...
          {purchaseOrder && (
            <div className="mb-6">
              ...
              <p>
                <strong>仕入ID:</strong> {purchaseOrder.purchase_order_id}
              </p>
              <p>
                <strong>注文日:</strong> {purchaseOrder.order_date}
              </p>
              <p>
                <strong>仕入先ID:</strong> {purchaseOrder.supplier_id}
              </p>
              <p>
                <strong>注文ステータス:</strong> {purchaseOrder.order_status}
              </p>
            </div>
          )}
